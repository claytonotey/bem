...
public:
  ...
  LK1Integrand(Vector3 &rt, GreensFunction *g, vector<RWGIndexPair> &s) : rt(rt), g(g), s(s) {
    memset(Gphi,0,9*sizeof(Complex));
  }
...
public:
  ...
  LK2Integrand(Vector3 &rt, GreensFunction *g, vector<RWGIndexPair> &s) : rt(rt), g(g), s(s) {
...
class LKSameNCrossOuterIntegrand : public TriangleIntegrand {
public:
  GreensFunction *g;
  vector<RWGIndexPair> &t;
  vector<RWGIndexPair> &s;
  ...
  LKSameNCrossOuterIntegrand(GreensFunction *g, vector<RWGIndexPair> &t, vector<RWGIndexPair> &s, Complex *L, Complex *K) 
    : g(g), t(t),s(s),L(L),K(K) {}
...
class LKSameDotOuterIntegrand : public TriangleIntegrand {
public:
  GreensFunction *g;
  vector<RWGIndexPair> &t;
  vector<RWGIndexPair> &s;
  ...
  LKSameDotOuterIntegrand(GreensFunction *g, vector<RWGIndexPair> &t, vector<RWGIndexPair> &s, Complex *L, Complex *K) 
    : g(g), t(t),s(s),L(L),K(K) {}
  ...
  void weighPoint(Real w, Vector3 &v) {
    ...
    for(unsigned int i=0; i<t.size(); i++) {
      ...
      for(unsigned int j=0; j<s.size(); j++) {
        if(t[i].first->t == s[j].first->t) {
          L[ij] += w * Ai2 * dot(phi,(v - *s[j].first->p0));
        }
        ij++;
      }
    }
...
void LKTriangleNcross(GreensFunction *g, Triangle *trit, Triangle *tris, vector<RWGIndexPair> &t, vector<RWGIndexPair> &s, ComplexDenseMatrix &Z, Options &options)
{
  Complex L[9];
  Complex K[9];
  memset(L,0,9*sizeof(Complex));
  ...
  if(trit == tris) {
    ...
    LKSameNCrossOuterIntegrand outerintegrand(g,t,s,L,K);
...
void LKTriangleDot(GreensFunction *g, Triangle *trit, Triangle *tris, vector<RWGIndexPair> &t, vector<RWGIndexPair> &s, ComplexDenseMatrix &Z, Options &options)
{
  Complex L[9];
  Complex K[9];
  memset(L,0,9*sizeof(Complex));
  ...
  if(trit == tris) {
    TriangleQuadratureGauss105 outerquad;
    LKSameDotOuterIntegrand outerintegrand(g,t,s,L,K);
    outerquad.integrate(outerintegrand,*th.p0,th.v01,th.v02);
  }
         ...
         Real gap)
{
  ...
  for(TriangleRWGMap::iterator i = tt.begin(); i != tt.end(); ++i) {
    ...
    for(int j=0; j<cols; j++) {
      ...
      LK1Integrand integrand(ps[sindex], g, t);
                              ...
                              Options &options)
{
  ...
  for(TriangleRWGMap::iterator i = tt.begin(); i != tt.end(); ++i) {
    for(TriangleRWGMap::iterator j = ss.begin(); j != ss.end(); ++j) {
      LKTriangleNcross(g,i->first,j->first,i->second,j->second,*dense,options);
    }
  }
  return dense;
}
                             ...
                             Options &options)
{
  if(M->type == ComplexMatrix::dense) {
    ...
    Zrk(t,s,rk,g,cache,cacheLock,options,gap);
    omp_set_lock(progressLock);
