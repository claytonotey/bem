...
SurfaceParameterization *RWGTree :: parameterize()
{
  PointSet point;
  PointSet boundaryPoint;
  ...
  for(EdgeSet::const_iterator i = boundary.begin(); i != boundary.end(); ++i) {
    ...
    point.insert(e->p1);
    point.insert(e->p2);
    boundaryPoint.insert(e->p1);
    boundaryPoint.insert(e->p2);
    boundaryEdgesAtPoint[e->p1].push_back(e);
    boundaryEdgesAtPoint[e->p2].push_back(e);
  }

  return new SurfaceParameterization(point, boundaryPoint, edgesAtPoint, boundaryEdgesAtPoint, triangle);
}
  ...
  : edgesAtPoint(edgesAtPoint_)
{
  ...
  for(ShapeMeshDielectricPairList::const_iterator j = meshDiel.begin(); j != meshDiel.end(); ++j) {
    ...
    GreensFunction *g = new GreensFunction(diel,false);
