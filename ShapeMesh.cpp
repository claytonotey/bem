...
void ShapeMesh :: generateVolumeMesh(VolumeMesh &vMesh)
{
  tetgenio in, out;
  tetgenio::facet *f;
  tetgenio::polygon *p;
  ...
  in.numberofpoints = getPointCount();
  in.pointlist = new Real[in.numberofpoints * 3];

  UnorderedMap< Point*, int> pointIndex;
  ...
  for(vector< Array<Point>* >::const_iterator i = refinedPoint.begin(); i != refinedPoint.end(); ++i) {
    ...
    for(int j=0; j<point.n; j++) {
      pointIndex[point+j] = jjj++;
      in.pointlist[jj++] = point[j].x;
      in.pointlist[jj++] = point[j].y;
      in.pointlist[jj++] = point[j].z;
    }
  }

  in.numberoffacets = getTriangleCount();
  ...
  for(vector< Array<Triangle>* >::const_iterator i = refinedTriangle.begin(); i != refinedTriangle.end(); ++i) {
    ...
    for(int j=0; j<triangle.n; j++) {
      ...
      p->vertexlist[0] = pointIndex[t->p1];
      p->vertexlist[1] = pointIndex[t->p2];
      p->vertexlist[2] = pointIndex[t->p3];
      in.facetmarkerlist[jj] = 0;
      jj++;
    }
  }
  ...
  const char *cmd = "pQq1.2";
  tetrahedralize((char*)cmd, &in, &out);

  UnorderedMap<int, Point*> indexPoint;

  vMesh.point.resize(out.numberofpoints);
  vMesh.tetrahedra.resize(out.numberoftetrahedra);

  jj = 0;
  for(int i=0; i<out.numberofpoints; i++) {
    vMesh.point[i].x = out.pointlist[jj++];
    vMesh.point[i].y = out.pointlist[jj++];
    vMesh.point[i].z = out.pointlist[jj++];
    indexPoint[i] = vMesh.point + i;
  }

  jj = 0;
  for(int i=0; i<out.numberoftetrahedra; i++) {
    vMesh.tetrahedra[i].p1 = indexPoint[out.tetrahedronlist[jj++]];
    vMesh.tetrahedra[i].p2 = indexPoint[out.tetrahedronlist[jj++]];
    vMesh.tetrahedra[i].p3 = indexPoint[out.tetrahedronlist[jj++]];
    vMesh.tetrahedra[i].p4 = indexPoint[out.tetrahedronlist[jj++]];
  }
